...
class Application_Model_Task_Magento_Install2
...
if(stristr($_SERVER[\'REQUEST_URI\'], \'setting\')) {
    ...
    protected function _runInstaller() {
        ...
        $composerCommand = $this->cli('composer')->installRoot($this->_storeFolder . '/' . $this->_domain);
        $output = $composerCommand->call()->getLastOutput();
        $message = var_export($output, true);
        $this->logger->log("\n" . $composerCommand. "\n" . $message, Zend_Log::DEBUG);
        unset($output);

        $composerCommand = $this->cli('composer')->installSetup($this->_storeFolder . '/' . $this->_domain . '/setup/');
        $output = $composerCommand->call()->getLastOutput();
        $message = var_export($output, true);
        $this->logger->log("\n" . $composerCommand. "\n" . $message, Zend_Log::DEBUG);
        unset($output);
        ...
        $command = $this->cli()->createQuery('cd ?', $this->_storeFolder . '/' . $this->_domain .'/setup');
        $install = $this->cli()->createQuery(
            '/usr/bin/php -f index.php install '.
            ' --language=en_US' .
