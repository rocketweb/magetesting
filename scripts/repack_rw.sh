...
if [ -z $FILE ]; then
    ...
    $IONCUBE --allowed-server magetesting.com,*.magetesting.com --obfuscate all --obfuscation-key "$KEY" --obfuscation-ex ${DIR}/ioncube.blist --ignore .svn/ --ignore .DS_Store  --encode "*.php" --encode "*.phtml" ${BASEDIR}/temp -o ${BASEDIR}/temp-encoded
