...
[ ! -e "$SOURCE_ARCHIVE" ] && {
    echo -e "Usage:\n$0 magento-sample-data-1.x.x.x.tar.gz"
    exit 2
}
...
mv ../$SOURCE_ARCHIVE ../$SOURCE_ARCHIVE.uncompressed
tar -czf "../$SAMPLE_DATA_DIR.tar.gz" "$SAMPLE_DATA_DIR"
cd .. # get out of the tmp-work-dir
rm -r "$WORK_DIR"

NEW_SIZE=$(du -sh "$SAMPLE_DATA_DIR.tar.gz" | awk '{ print $1 }');
echo "New compressed sample data archive: $SAMPLE_DATA_DIR.tar.gz"
